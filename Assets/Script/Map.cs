...
public class Map : MonoBehaviour
{
    ...
    void BoundaryCheck()
    {
        if (player.position.x - transform.position.x > 50) // 플레이어의 x좌표와 맵의 좌표를 뺐을때 맵의 절반보다 클때 // 오른쪽으로 갈때
        {
            MoveLand(2);
        }
        if (player.position.x - transform.position.x < -LandSize / 2) // 왼쪽으로 갈때
        {
            MoveLand(0);
            
        }
        if (player.position.z - transform.position.z > LandSize / 2) // 위로 갈때
        {
            MoveLand(1);
            
        }
        if (player.position.z - transform.position.z < -LandSize / 2) // 아래로 갈때
        {
            MoveLand(3);
        }
    }
